...
$RepoUrlSsh = "git@github.com:402617833/Hermes-Desktop-CN.git"
$RepoUrlHttps = "https://github.com/402617833/Hermes-Desktop-CN.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/402617833/Hermes-Desktop-CN/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/402617833/Hermes-Desktop-CN/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/402617833/Hermes-Desktop-CN/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
...
function Install-Desktop {
    ...
    try {
        ...
        Write-Info "Patching desktop build config..."
        & node scripts/patch-build-config.cjs 2>&1 | ForEach-Object { "$_" }
        $patchCode = $LASTEXITCODE
        if ($patchCode -ne 0) { throw "desktop build config patch failed (exit $patchCode)" }
        ...
        if ($code -ne 0) {
            ...
            if ($purged.Count -gt 0) {
                ...
                Write-Info "Re-patching desktop build config before retry..."
                & node scripts/patch-build-config.cjs 2>&1 | ForEach-Object { "$_" }
                $patchCode = $LASTEXITCODE
                if ($patchCode -ne 0) { throw "desktop build config patch failed before retry (exit $patchCode)" }
                & $npmExe run pack 2>&1 | ForEach-Object { "$_" } | Tee-Object -FilePath $buildLog
                $code = $LASTEXITCODE
            }
        }
